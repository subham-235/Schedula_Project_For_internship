...
export default function MyAppointmentsPage() {
  ...
  const downloadPrescription = (
    ...
    pdf.text("Schedula", 16, 14);

    pdf.setFontSize(10);

    pdf.setFont("helvetica", "normal");

    pdf.text("Medical Prescription", 16, 22);
    ...
    const field = (label: string, value: string) => {
      ...
      pdf.text(lines, 16, y + 5);

      y += 13 + Math.max(0, lines.length - 1) * 4;
    };

    field("Patient", booking.patientName);

    field("Doctor", booking.doctorName);

    field("Specialty", booking.specialty);

    field(
      "Appointment",
      `${formatAppointmentFullDate(booking.startsAt)} · ${formatAppointmentTime(
        booking.startsAt,
      )}`,
    );

    field("Diagnosis", prescription.diagnosis);

    field(
      "Medications",
      prescription.medications.length > 0
        ? prescription.medications
            .map((medicine, index) => `${index + 1}. ${medicine}`)
            .join("\n")
        : "No medications listed.",
    );

    field("Advice / Notes", prescription.notes || "No additional notes.");

    pdf.setFontSize(8);

    pdf.setFont("helvetica", "normal");

    pdf.setTextColor(100, 110, 106);

    pdf.text(
      `Generated by Schedula on ${new Intl.DateTimeFormat("en-IN", {
        dateStyle: "medium",
      }).format(new Date())}`,
      16,
      280,
